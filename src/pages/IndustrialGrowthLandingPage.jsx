...
export default function IndustrialGrowthLandingPage() {
  ...
  // Live Secure Server-Side Submission Handler (Gate B)
  const handleSubmit = async (e) => {
    ...
    // 4. Construct payload matching canonical contract
    const submissionPayload = {
      ...
      city: formData.city,
      primaryGrowthProblem: formData.primaryGrowthProblem,
      websiteUrl: formData.websiteUrl.trim() || null,
      linkedinUrl: formData.linkedinUrl.trim() || null,
      companyFaxExt: formData.companyFaxExt || '',
    ...
    try {
      const response = await fetch('/api/leads/industrial', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok || !result.success) {
        throw new Error(result.error || (isAr 
          ? 'تعذر معالجة الطلب حالياً. يُرجى إعادة المحاولة أو التواصل مباشرة عبر الهاتف.'
          : 'Unable to process screening request. Please retry or contact us directly.'));
      }
      ...
      console.error('[Industrial Intake Error]:', err);
      setSubmissionError(err.message || (isAr 
        ? 'حدث خطأ في الاتصال أثناء إرسال الطلب. تم حفظ بياناتك المدخلة، يُرجى إعادة المحاولة.'
        : 'A connection error occurred. Your entered data has been preserved, please retry.'));
    } finally {
          ...
          {submissionResult ? (
            <div className="border border-neutral-300 rounded-lg p-8 sm:p-10 text-center bg-white shadow-xs max-w-2xl mx-auto">
              ...
              <h3 className={`text-xl sm:text-2xl font-bold text-neutral-950 mb-3 ${fontSubheadClass}`}>
                {isAr ? 'تم استلام وتأكيد طلب الفحص التشخيصي بنجاح' : 'Diagnostic Screening Request Confirmed'}
              </h3>
              <p className={`text-sm text-neutral-600 mb-6 leading-relaxed ${fontBodyClass}`}>
                {isAr 
                  ? 'تم استلام طلب الفحص التشخيصي لشركتكم بنجاح وتسجيله في النظام التنفيذي. يقوم استشاري النمو الصناعي حالياً بإجراء التدقيق التمهيدي للبصمة الرقمية للشركة وسنتواصل معكم خلال 15 دقيقة لتأكيد موعد الجلسة ومناقشة تقرير الفحص الأولي.'
                  : 'Your company diagnostic screening request has been successfully received and confirmed. Our Industrial Growth Consultant is conducting the initial digital footprint audit and will connect with your team within 15 minutes to coordinate the session and discuss initial findings.'}
              </p>
              ...
              <div className="p-4 rounded bg-neutral-50 border border-neutral-200 text-xs text-start font-mono text-neutral-600 mb-8 font-lato">
                <div className="text-neutral-900 font-bold mb-1 pb-1 border-b border-neutral-200 flex justify-between items-center">
                  <span>{isAr ? 'المرجع الرسمي للطلب' : 'Official Request Reference'}</span>
                  <span className="font-mono text-neutral-950 font-bold">{submissionResult.leadId}</span>
                ...
                <div>Status: CONFIRMED & QUEUED</div>
              </div>
            ...
            <form onSubmit={handleSubmit} className="border border-neutral-200 rounded-lg p-8 sm:p-10 bg-white shadow-sm space-y-6 text-start">
              ...
              <div className="pt-2">
                {submissionError && (
                <div className="p-4 rounded-lg bg-red-50 border border-red-200 text-xs text-[#B5121B] flex items-start gap-2.5 leading-relaxed">
                  <AlertCircle className="w-4 h-4 shrink-0 mt-0.5 text-[#B5121B]" />
                  <div>
                    <div className="font-bold mb-0.5">{isAr ? 'تعذر إرسال الطلب' : 'Submission Failed'}</div>
